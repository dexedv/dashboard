...
const permissionsData = [
  ...
  { name: 'customers.upload_files', description: 'Dateien hochladen', category: 'customers' },
  { name: 'customers.delete_files', description: 'Dateien löschen', category: 'customers' },
  ...
  { name: 'employees.toggle_active', description: 'Aktivieren/Deaktivieren', category: 'employees' },
  ...
  // Admin
  { name: 'admin.access_panel', description: 'Admin-Panel Zugriff', category: 'admin' },
...
async function main() {
  ...
  if (admin) {
    ...
    const existingPermIds = new Set(existingUserPerms.map(p => p.permissionId));
    const newPerms = allPermissions.filter(p => !existingPermIds.has(p.id));
    ...
    if (newPerms.length > 0) {
      ...
      console.log(`Added ${newPerms.length} permissions to admin user`);
    } else {
